...
def sync_match(client: SheetsClient, match: MatchResult) -> Tuple[bool, bool]:
    updates_contaordem: List[Dict[str, Any]] = []
    updates_origem: List[Dict[str, Any]] = []
    ...
    if origem_id and origem_id != contaordem_id:
        updates_contaordem.append({"row": match.contaordem_row, ID_INTERNO_COL: origem_id})
        contaordem_changed = True

    if contaordem_doc and contaordem_doc != origem_doc:
        updates_origem.append({"row": match.t_extrato_row, DOC_COL: contaordem_doc})
        origem_changed = True

    updates_contaordem.append({"row": match.contaordem_row, VALIDATION_COL: "VALIDADO"})
    contaordem_changed = True

    if updates_contaordem:
        client.batch_update_rows(CONTAORDEM_SHEET, updates_contaordem)
    if updates_origem:
        client.batch_update_rows(T_EXTRATO_SHEET, updates_origem)

    return contaordem_changed, origem_changed


def run(limit: Optional[int] = None, dry_run: bool = False) -> ValidationReport:
    settings = _load_settings()
    client = SheetsClient(settings)
    ensure_validation_column(client, CONTAORDEM_SHEET, VALIDATION_COL)

    initial_contaordem_rows = _read_records(client, CONTAORDEM_SHEET)
    initial_t_extrato_rows = _read_records(client, T_EXTRATO_SHEET)

    report = summarize_rows(initial_contaordem_rows)
    report.t_extrato_lidas = len(initial_t_extrato_rows)

    processed = 0
    while limit is None or processed < limit:
        contaordem_rows = _read_records(client, CONTAORDEM_SHEET)
        t_extrato_rows = _read_records(client, T_EXTRATO_SHEET)
        ...
        print(
            f"Match encontrado | CONTAORDEM={match.contaordem_row} | "
            f"T_EXTRATO={match.t_extrato_row} | key={match.key}"
        )
        print(
            "Comparação | "
            f"ID_INTERNO={'OK' if match.id_interno_equal else 'DIFERENTE'} | "
            f"DOC. SOMA={'OK' if match.doc_equal else 'DIFERENTE'}"
        )
        ...
        contaordem_changed, origem_changed = sync_match(client, match)
        report.matches += 1
        ...
        print(
            ...
            f"CONTAORDEM linha {match.contaordem_row} marcada em {VALIDATION_COL} "
            f"e T_EXTRATO linha {match.t_extrato_row} ajustada quando necessário."
        )

    final_contaordem_rows = _read_records(client, CONTAORDEM_SHEET)
    remaining = 0
    for row in final_contaordem_rows:
...
def main() -> None:
    ...
    report = run(limit=args.limit, dry_run=args.dry_run)
    print_report(report)
