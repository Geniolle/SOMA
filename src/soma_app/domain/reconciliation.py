...
# Padrão: espaço(s) opcional(is) + N + um ou mais dígitos no FINAL da string.
_SOMA_SUFFIX_RE = re.compile(r"\s+N\d+$", re.IGNORECASE)


def normalize_soma_description(value: Any) -> str:
    """Remove o sufixo ``N<dígitos>`` acrescentado pelo SOMA ao final da
    descrição (ex.: ``DÍZIMOS E OFERTAS N004`` → ``DÍZIMOS E OFERTAS``).

    Não remove números que façam parte legítima do texto (ex.: ``LOTE 3`` ou
    ``REF 2024/001`` não são alterados porque o sufixo não segue o padrão
    exato ``espaço+N+dígitos`` no final).
    """
    text = normalize_text(value)
    return _SOMA_SUFFIX_RE.sub("", text).strip()
...
def reconcile_t_extrato(
    ...
    *,
    ...
    for source_ref in source_refs:
        ...
        # doc_soma para lookup no SOMA: usa apenas o valor da CONTAORDEM.
        # NÃO usamos source_doc como fallback — a CONTAORDEM é a fonte de
        # verdade para o DOC. SOMA; se não tiver um numérico, o lançamento
        # deve aparecer como NAO_ENCONTRADO_SOMA (ex.: EXT0000002976).
        item.doc_soma = conta_doc
        ...
        # Normalizar ambas as descrições removendo o sufixo Nxxx antes de
        # comparar. O campo DESCRIÇÃO SOMA da CONTAORDEM e o campo DESCRIÇÃO
        # da sheet SOMA ambos podem conter o sufixo (ex.: N001, N002…).
        soma_desc_norm = normalize_soma_description(soma.get("DESCRIÇÃO") or "")
        conta_desc_soma_norm = normalize_soma_description(conta.get("DESCRIÇÃO SOMA") or "")
        desc_conta_soma_issue: ReconciliationIssue | None = None
        if soma_desc_norm != conta_desc_soma_norm:
            desc_conta_soma_issue = ReconciliationIssue(
                code="DESCRICAO_CONTA_SOMA",
                message="DESCRIÇÃO CONTAORDEM↔SOMA divergente.",
                field="DESCRIÇÃO SOMA",
                origin_value=str(conta.get("DESCRIÇÃO SOMA") or ""),
